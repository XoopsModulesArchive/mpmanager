	...
	foreach ($_POST['perms'] as $perm_name => $perm_data) {
		foreach( $perm_data['itemname' ] as $item_id => $item_name ) {
			...
			if (false != myDeleteByModule($gperm_handler->db,$modid,$perm_name,$item_id)) {
				...
				if ($perm_name == 'module_read') {
$sq1 = "INSERT INTO ".$xoopsDB->prefix("group_permission")."(gperm_id, gperm_groupid, gperm_itemid, gperm_modid, gperm_name) VALUES(0, $group_id, 1, 1, 'module_read')";
	...
	//				foreach ($item_ids as $item_id => $selected) {
					...
					if ($selected == 1) {
						...
						if (!$gperm_handler->insert($gperm)) {
							$msg[] = sprintf('<b>'.$perm_name.'</b>', '<b>'.$perm_data['itemname'][$item_id].'</b>', '<b>'.$group_list[$group_id].'</b>');
						} else {
							$msg[] = sprintf('<b>'.$perm_name.'</b>', '<b>'.$perm_data['itemname'][$item_id].'</b>', '<b>'.$group_list[$group_id].'</b>');
						}
						unset($gperm);
