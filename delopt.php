...
$_SESSION['ct_file'] =& $_REQUEST['ct_file'];

	
if (@$_REQUEST['option'] == 'delfile' && isset($_REQUEST['ct_file'])) 
 {
  xoops_confirm(array('catbox' => $catbox, 'option' => 'delfiles'), 'delopt.php', _MP_IMSURFILE);
 }
 else {
 redirect_header("javascript:history.go(-1)",2, _PM_REDNON);
 }
...
if (@$_REQUEST['option'] == 'delfiles' && isset($_SESSION['ct_file'])) 
 {
 
 $pm_handler  = & xoops_gethandler('priv_msgscat');
 $pm =& $pm_handler->get($_SESSION['ct_file']);
 $erreur = $pm_handler->delete($pm);
 
 $pm_handler2  = & xoops_gethandler('priv_msgs');
 $criteria = new CriteriaCompo();
 $criteria->add(new Criteria('cat_msg', $_SESSION['ct_file']));
 $erreur .= $pm_handler2->deleteAll($criteria);
 if (!$erreur) {
 redirect_header("filebox.php",2, _PM_REDNON);
 } else {
 redirect_header("filebox.php",2, _MP_FILEDELETED);
 }
